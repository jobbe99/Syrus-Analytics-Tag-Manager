    ...
    <table class="gadwp-settings-options">
        <tr>
            <td colspan="2" class="gadwp-settings-info">
						<?php echo __( "Utilizza questo link per ricevere il tuo access code:", 'syrus-analytics-tag-manager' ) . ' <a href="' . $authUrl . '" id="gapi-access-code" target="_blank">' . __ ( "Ricevi Access Code", 'syrus-analytics-tag-manager' ) . '</a>.'; ?>
					</td>
        </tr>
        <tr>
            <td class="gadwp-settings-title">
                <label for="ga_dash_code" title="<?php _e("Utilizza il link rosso per accedere al tuo access code!",'syrus-analytics-tag-manager')?>"><?php echo _e( "Access Code:", 'syrus-analytics-tag-manager' ); ?></label>
            </td>
            <td>
                <input type="text" id="ga_dash_code" name="ga_dash_code" value="" size="61" required="required" title="<?php _e("Use the red link to get your access code!",'syrus-analytics-tag-manager')?>">
            </td>
        </tr>
        <tr>
            <td colspan="2">
        ...
        <tr>
            <td colspan="2">
                <input type="submit" class="button button-secondary" name="ga_dash_authorize" value="<?php _e( "Salva Access Code", 'syrus-analytics-tag-manager' ); ?>" />
            </td>
        </tr>
    </table>
</form>
<?php
		...
		private function get_areachart_data( $projectId, $from, $to, $query, $filter = '' ) {
			switch ( $query ) {
				case 'users' :
					$title = __( "Users", 'syrus-analytics-tag-manager' );
					break;
				case 'pageviews' :
					$title = __( "Page Views", 'syrus-analytics-tag-manager' );
					break;
				case 'visitBounceRate' :
					$title = __( "Bounce Rate", 'syrus-analytics-tag-manager' );
					break;
				case 'organicSearches' :
					$title = __( "Organic Searches", 'syrus-analytics-tag-manager' );
					break;
				case 'uniquePageviews' :
					$title = __( "Unique Page Views", 'syrus-analytics-tag-manager' );
					break;
				default :
			...
			if ( $from == "today" || $from == "yesterday" ) {
				$dimensions = 'ga:hour';
				$dayorhour = __( "Hour", 'syrus-analytics-tag-manager' );
			} else if ( $from == "365daysAgo" || $from == "1095daysAgo" ) {
				$dimensions = 'ga:yearMonth, ga:month';
				$dayorhour = __( "Date", 'syrus-analytics-tag-manager' );
			} else {
			...
			if ( $from == "today" || $from == "yesterday" ) {
				...
				foreach ( $data->getRows() as $row ) {
					 ...
					 */
					$gadwp_data[] = array( date_i18n( __( 'F, Y', 'syrus-analytics-tag-manager' ), strtotime( $row[0] . '01' ) ), round( $row[2], 2 ) );
				}
			} else {
				foreach ( $data->getRows() as $row ) {
					/*
					 ...
					 * For details see: http://php.net/manual/en/function.date.php#refsect1-function.date-parameters
					 */
					$gadwp_data[] = array( date_i18n( __( 'l, F j, Y', 'syrus-analytics-tag-manager' ), strtotime( $row[0] ) ), round( $row[2], 2 ) );
				}
			}
		...
		private function get_contentpages( $projectId, $from, $to, $filter = '' ) {
			...
			$gadwp_data = array( array( __( "Pages", 'syrus-analytics-tag-manager' ), __( "Sessions", 'syrus-analytics-tag-manager' ) ) );
			foreach ( $data->getRows() as $row ) {
		...
		private function get_404errors( $projectId, $from, $to, $filter = "Page Not Found" ) {
			...
			$gadwp_data = array( array( __( "404 Errors", 'syrus-analytics-tag-manager' ), __( "Sessions", 'syrus-analytics-tag-manager' ) ) );
			foreach ( $data->getRows() as $row ) {
				$path = esc_html( $row[0] );
				$source = esc_html( $row[1] );
				$gadwp_data[] = array( "<strong>" . __( "URI:", 'syrus-analytics-tag-manager' ) . "</strong> " . $path . "<br><strong>" . __( "Source:", 'syrus-analytics-tag-manager' ) . "</strong> " . $source, (int) $row[2] );
			}
			return $gadwp_data;
		}
		...
		private function get_referrers( $projectId, $from, $to, $filter = '' ) {
			...
			$gadwp_data = array( array( __( "Referrers", 'syrus-analytics-tag-manager' ), __( "Sessions", 'syrus-analytics-tag-manager' ) ) );
			foreach ( $data->getRows() as $row ) {
				$gadwp_data[] = array( esc_html( $row[0] ), (int) $row[1] );
			}
			return $gadwp_data;
		...
		private function get_searches( $projectId, $from, $to, $filter = '' ) {
			...
			$gadwp_data = array( array( __( "Searches", 'syrus-analytics-tag-manager' ), __( "Sessions", 'syrus-analytics-tag-manager' ) ) );
			foreach ( $data->getRows() as $row ) {
				$gadwp_data[] = array( esc_html( $row[0] ), (int) $row[1] );
			}
			return $gadwp_data;
		...
		private function get_locations( $projectId, $from, $to, $filter = '' ) {
			$metrics = 'ga:sessions';
			$options = "";
			$title = __( "Countries", 'syrus-analytics-tag-manager' );
			...
			if ( $this->gadwp->config->options['ga_target_geomap'] ) {
				...
				if ( isset( $country_codes[$this->gadwp->config->options['ga_target_geomap']] ) ) {
					...
					$title = __( "Cities from", 'syrus-analytics-tag-manager' ) . ' ' . __( $country_codes[$this->gadwp->config->options['ga_target_geomap']] );
			...
			$gadwp_data = array( array( $title, __( "Sessions", 'syrus-analytics-tag-manager' ) ) );
			foreach ( $data->getRows() as $row ) {
				if ( isset( $row[2] ) ) {
		...
		private function get_orgchart_data( $projectId, $from, $to, $query, $filter = '' ) {
			...
			$block = ( $query == 'channelGrouping' ) ? __( "Channels", 'syrus-analytics-tag-manager' ) : __( "Devices", 'syrus-analytics-tag-manager' );
		...
		private function get_piechart_data( $projectId, $from, $to, $query, $filter = '' ) {
			...
			$gadwp_data = array( array( __( "Type", 'syrus-analytics-tag-manager' ), __( "Sessions", 'syrus-analytics-tag-manager' ) ) );
			$i = 0;
			$included = 0;
			foreach ( $data->getRows() as $row ) {
				if ( $i < 20 ) {
			...
			if ( $others > 0 ) {
				$gadwp_data[] = array( __( 'Other', 'syrus-analytics-tag-manager' ), $others );
			}
		...
		public function frontend_widget_stats( $projectId, $from, $anonim ) {
			...
			$gadwp_data = array( array( __( "Date", 'syrus-analytics-tag-manager' ), __( "Sessions", 'syrus-analytics-tag-manager' ) ) );
			if ( $anonim ) {
				$max_array = array();
			...
			foreach ( $data->getRows() as $row ) {
				$gadwp_data[] = array( date_i18n( __( 'l, F j, Y', 'syrus-analytics-tag-manager' ), strtotime( $row[0] ) ), ( $anonim ? round( $row[2] * 100 / $max, 2 ) : (int) $row[2] ) );
			}
			$totals = $data->getTotalsForAllResults();
